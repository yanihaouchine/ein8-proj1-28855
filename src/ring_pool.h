...
static inline __attribute__((always_inline)) void ring_init(void)
{
    ring_idx.head = 0;
    ring_idx.tail = 0;
}

static inline __attribute__((always_inline)) int ring_empty(void)
{
    return ring_idx.head == ring_idx.tail;
}

static inline __attribute__((always_inline)) void ring_put_last(thread_hot_t *th)
{
...
static inline __attribute__((always_inline)) thread_hot_t *ring_remove_first(void)
{
...
static inline __attribute__((always_inline)) thread_hot_t *ring_remove_last(void)
{
    ring_idx.tail--;
