...
void sched_init() { STAILQ_INIT(&ready_queue); }

void sched_enqueue(thread_m *t) { STAILQ_INSERT_TAIL(&ready_queue, t, link); }

int is_sched_empty(void) { return STAILQ_EMPTY(&ready_queue); }
