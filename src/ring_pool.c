...
//   - Zéro allocation dynamique après la phase de croissance
//   - Doublement automatique de la capacité quand le buffer est plein
...
static inline int phys(pool *p, int i) { return (p->head + i) % p->cap; }

// Relinéarise et double la capacité du buffer.
static int pool_grow(pool *p) {
  int new_cap = p->cap * 2;
  thread_m **new_data = malloc(sizeof(thread_m *) * new_cap);
  if (new_data == NULL) {
    LOG_E("Echec de l'allocation lors du redimensionnement (new_cap=%d)",
          new_cap);
    return -1;
  }
  for (int i = 0; i < p->size; i++)
    new_data[i] = p->data[phys(p, i)];
  free(p->data);
  p->data = new_data;
  p->head = 0;
  p->cap = new_cap;
